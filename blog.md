...
Let's leave the third section (the one with `user_id` and `group_id` ARGs, skipped from the listing above) aside for a moment, we'll come back there later.
...
The only way to prevent bloating the resulting image is to remove no longer necessary files in the same layer as they were added.
...
What survives in the resulting layer is only the git installation that we care for, but not the GNU toolchain necessary to get this installation sorted in the first place
...
By the way, since the YML itself is located in ci/tox/, the assumption is that `${PWD}` points to ci/tox/ rather than to the project root.
...
That caching was by the way also the purpose of the intense size optimization outlined previously.
...
We definitely need to include those version in the Docker image tag to make the caching possible.
...
But since all those files reside under ci/tox/, this doesn't make things very worse - this makes the entire resulting build image depend only on the contents of ci/tox/ directory
...
We've taken a look at the entire stack used for building and testing the git branches,
